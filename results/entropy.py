...
for bird, ax_ind in zip(bird_list, range(len(bird_list))):
    ...
    sns.lineplot(x='taskSessionDeafening', y='entropyVarUndir', hue='note',
...
# plt.suptitle(f'EV FR >= {fr_crit} (Undir) ', y=.9, fontsize=15)
#
# for ind, task in enumerate(sorted(df['taskName'].unique(), reverse=True)):
#     ax = plt.subplot2grid((5, 2), (1, ind), rowspan=4, colspan=1)
...
# oddsratio, pvalue = fisher_exact([[3, 9], [18, 30]])
# pvalue