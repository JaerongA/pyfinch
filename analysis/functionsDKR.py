...
def get_half_width(wf_ts_interp, avg_wf_interp):
    import numpy as np
    import pandas as pd
    df = pd.DataFrame(avg_wf_interp)

    """
    Old calculation for half-width
    ...
    # Define baseline
    df_baseline = df.rolling(100, min_periods=100).mean()

    #Calculate Standard Deviation of baseline
    df_baselineSD_rolling = []
    df_baselineSD_rolling = df.rolling(100, min_periods=100).std()
    df_baselineSD_ = df_baselineSD_rolling
            # Todo: HOW DO I SET BASELINE SD AS FIRST WINDOW OF 100 TIMEPOINTS??? ###



    #Start of Negative Deflection
    def_start = []
    def_start = np.where(df.rolling(10, min_periods = 10).mean() < (df_baseline - df_baselineSD))
                    #Sets deflection baseline as 1 StdDev below Baseline (100 'frames')

    #Peak Minima
    #peak_minima = df.min(avg_wf_interp)
    def_end = []
    def_end = np.where(df.min(avg_wf_interp))

    half_width = (wf_ts[def_end] - wf_ts[def_start]) / 2
