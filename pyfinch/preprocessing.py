...
def intan2wav(data_path=None, *args):
    ...
    for rhd in rhd_files:
        ...
        fig, ax = plt.subplots(nrows=nb_channels + 1, ncols=1, sharex=True,
                               figsize=(8, 2 * nb_channels)
                               )
        ...
        spect, spect_freq, _ = spectrogram(intan['board_adc_data'][0], samp_freq=sample_rate, freq_range=freq_range,
                                           transform_type='log_spect'
                                           )
        spect_time = np.linspace(intan['t_amplifier'][0], intan['t_amplifier'][-1],
                                 spect.shape[1])  # timestamp for spectrogram

        ax[0].pcolormesh(spect_time, spect_freq, spect,
        ...
        ax[0].spines['left'].set_visible(False), ax[0].spines['bottom'].set_visible(False)
...
def rhd(data_path=None, save_fig=True,
        fig_ext='.png', view_folder=True
        ):
    """
