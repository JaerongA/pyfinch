"""
Analyze unit profiles including waveform characteristics, burstiness of a neuron, etc
Loop through cluster table
Store results in unit_profile table
"""
...
# Create a new database (unit_profile)
...
if update_db:
    ...
    db.cur.execute('''INSERT OR IGNORE INTO unit_profile (clusterID) VALUES (?)''', (row["id"],))
    db.cur.execute('''UPDATE unit_profile SET (birdID, taskName, taskSession, site, channel, unit, region) = \
