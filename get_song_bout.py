...
for cluster_run in range(0, summary_df.shape[0]):
    ...
    cur.execute("UPDATE song SET songBoutUndir = ? WHERE id = ?", (bout['Undir']['notes'], int(cluster.Key)))
    cur.execute("UPDATE song SET songBoutDir = ? WHERE id = ?", (bout['Dir']['notes'], int(cluster.Key)))
