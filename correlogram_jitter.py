...
for row in db.cur.fetchall():
    ...
    # Get correlogram shuffling baseline by using jittered spikes (to check peak significance)
    # corr_arr = []
    #
    # for iter in range(shuffle_iter):
    #     mi.jitter_spk_ts()
    #     correlogram_jitter = mi.get_correlogram(mi.spk_ts_jittered, mi.spk_ts_jittered)
    #     corr_arr.append(correlogram_jitter['U'])
    #
    # corr_arr = np.array(corr_arr)
    # corr_mean = corr_arr.mean(axis=0)
    # corr_std = corr_arr.std(axis=0)
    # upper_lim = corr_mean + (corr_std * 2)
    # lower_lim = corr_mean - (corr_std * 2)
    # # Check peak significance
    # if corr.peak_value > upper_lim[corr.peak_ind]:
    #     category = 'Bursting'
    # else:
